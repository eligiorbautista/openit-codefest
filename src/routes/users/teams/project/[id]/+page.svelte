<script>
    ...
    onMount(() => {
        ...
        if (projectId) {
            ...
            if (currentProject) {
                currentTeam = mockTeams.find(team => team.id == currentProject.team_id);
    ...
    const goBack = () => {
        if (currentProject && currentTeam) {
            goto(`/users/teams/${currentTeam.id}`);
        } else {
            goto('/users/teams');
        }
    };
...
<svelte:head>
    <title>{currentProject?.name || 'Project'} - Kanban Board - {PUBLIC_APP_NAME}</title>
</svelte:head>
        ...
        <div class="max-w-7xl mx-auto px-6 py-4">
            <div class="flex items-center justify-between">
                    ...
                    {#if currentProject}
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-gray-600 text-sm">{currentProject.description}</p>
                            </div>
                        </div>
                    {/if}
                </div>
                ...
                <div class="flex items-center space-x-4">
                    {#if currentProject}
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {currentProject.is_archived ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}">
        ...
        <div class="max-w-7xl mx-auto">
            {#if currentProject}
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    <div class="bg-white rounded-lg p-4 shadow-sm border">
                        <div class="text-2xl font-bold text-gray-900">{projectBoards.length}</div>
                        <div class="text-sm text-gray-500">Boards</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 shadow-sm border">
                        <div class="text-2xl font-bold text-blue-600">{projectCards.length}</div>
                        <div class="text-sm text-gray-500">Total Cards</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 shadow-sm border">
                        <div class="text-2xl font-bold text-green-600">{projectCards.filter(card => !card.is_archived).length}</div>
                        <div class="text-sm text-gray-500">Active Cards</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 shadow-sm border">
                        <div class="text-2xl font-bold text-orange-600">{projectCards.filter(card => card.is_approved).length}</div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    <h2 class="text-xl font-semibold text-gray-900 mb-6">Project Boards</h2>
                            ...
                            {#each projectBoards as board}
                                <div class="bg-gray-50 rounded-lg p-4 min-h-[500px]">
                                    ...
                                    <div class="space-y-3">
                                        {#each getCardsForBoard(board.id) as card}
                                            <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200 hover:shadow-md transition-shadow cursor-pointer">
                                                ...
                                                <div class="flex items-start justify-between mb-3">
                                                    <h4 class="text-sm font-semibold text-gray-900 flex-1">{card.name}</h4>
                                        ...
                                        <!-- Add Card Button -->
                                        <button class="w-full bg-gray-100 hover:bg-gray-200 rounded-lg p-3 text-sm text-gray-600 font-medium transition-colors">
                                            + Add a card
                                        </button>
                                    </div>
                                </div>
                    ...
                    {:else}
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-lg font-medium text-gray-900 mb-2">No boards found</h3>
                            <p class="text-gray-500 mb-6">This project doesn't have any boards yet.</p>
            ...
            {:else}
                ...
                <div class="bg-white rounded-xl shadow-lg p-12 text-center">
                    ...
                    <p class="text-gray-500 mb-6">The project you're looking for doesn't exist or has been moved.</p>
                    <button on:click={goBack} class="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors">
                        Go Back
                    </button>
                </div>
            {/if}
